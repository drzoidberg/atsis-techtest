...
export default function fetchRandomQuoteByAuthor(authorStr) {
  return async function (dispatch) {
    try {
      const response = await breakingBadApi.get(`/quote/random?author=${authorStr}`)
      dispatch({
        type: FETCH_RANDOM_QUOTE_BY_AUTHOR,
        payload: {
          status: fulfilled,
          data: response.data,
